...
const packageJsonFile = fs.readFileSync(
  path.relative(process.cwd(), "package.json"),
  "utf8"
);
...
const cssPath = path.resolve(
  outputDir,
  `FeatherWiki-plumage_${packageJson.nickname}.css`
);
fs.writeFileSync(cssPath, cssOutput);
const outputCssKb =
  (Uint8Array.from(Buffer.from(cssOutput)).byteLength * 0.000977).toFixed(3) +
  " kilobytes";
  ...
  return esbuild
                  ...
                  shouldMinify(template) {
                    return (
                      ...
                      template.parts.some((part) =>
                        part.text.includes("<!DOCTYPE html>")
                      )
                    );
                  },
                });
    ...
    .then(async (result) => {
      ...
      for (const out of result.outputFiles) {
        ...
        if (/\.js$/.test(out.path)) {
          const jsFilename = `FeatherWiki-bones_${packageJson.nickname}${
            localeName !== "en-US" ? `_${localeName}` : ""
          }.js`;
          const jsOutPath = path.resolve(outputDir, jsFilename);
          fs.writeFileSync(jsOutPath, output);
          const jsKb =
            (
              Uint8Array.from(Buffer.from(output)).byteLength * 0.000977
            ).toFixed(3) + " kilobytes";
    ...
    .then(async (html) => {
      ...
      const filename = `FeatherWiki_${packageJson.nickname}${
        localeName !== "en-US" ? `_${localeName}` : ""
      }.html`;
      const filePath = path.resolve(outputDir, filename);
      const outHtml = minify(html, minifyOptions);
      const outputKb =
        (Uint8Array.from(Buffer.from(outHtml)).byteLength * 0.000977).toFixed(
          3
        ) + " kilobytes";
...
locales
  .filter((locale) => !locale.startsWith("en-US") && locale.endsWith(".json"))
